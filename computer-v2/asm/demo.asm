...
TERMINAL        equ 0x3C                ; Константа с адресом для вывода в терминал в текстовом
                                        ;   режиме

                jmp init                ; Переходим через область данных

; Верхняя часть картинки для вывода в терминал
img1        db  0b00000000,             ;                  ;
                0b00000000,             ;                  ;
                0b10000000,             ; ██               ;
                0b01100000,             ;   ████           ;
...
; Подготавливаем вывод нижней половинки картинки
img2_show:      ldi a, 0x19             ; Код инструкции "js c"
                st a, img_cond          ; Заменяем условие для выхода из цикла "img_loop"
                ldi b, img2_last        ; Указатель на последний байт картинки. В течение цикла
                                        ;   указатель смещается к первому байту и одновременно
                                        ;   работает как счётчик, по которому цикл завершается.
                jmp img_show

; Подготавливаем вывод верхней половинки картинки
img1_show:      ldi b, img1_last        ; Указатель на последний байт картинки. В течение цикла
                                        ;   указатель смещается к первому байту и одновременно
                                        ;   работает как счётчик, по которому цикл завершается.
...
; Выводим картинку в терминал при помощи цикла
img_loop:       ld a, b
                st a, d
                dec b
...
; Нижняя часть картинки для вывода в терминал
img2        db  0b00000000,             ;                  ;
                0b00000000,             ;                  ;
                0b00011110,             ;       ████████   ;
                0b00100001,             ;     ██        ██ ;
...
text2_show:     ldi a, img2_show
                ...
                ldi a, pre_end
                st a, img_jmp +1        ; Заменяем адрес для перехода после вывода картинки
                ldi b, text2
                ldi c, text2_size
