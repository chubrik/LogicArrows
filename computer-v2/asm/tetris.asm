...
BANK_TURN_2     equ 3                       ; Продолжение поворота фигуры и данные о повортах
...
; 0x00, т.к. большинство алгоритмов работают от конца к началу, используя буфер ещё и в качестве
; счётчика в цикле.
...
; Выставляем флаг необходимости блокировки фигуры и готовим отдельные указатели, на случай если шаг
; будет прерван
...
; Сбрасываем флаг необходимости блокировки фигуры и переходим к следующему шагу
                    st a, need_block
                    jmp step_next
...
left_copy_loop:     ld a, d
                    ...
                    ld a, d
                    rcl a
                    jc step_next            ; Если вышли за левый край, преходим к следующему шагу
                    st a, c
                    dec d
                    dec c
                    jns left_copy_loop
...
; Продолжение поворота фигуры и данные о повортах
...
; Если сработало одно из ограничений, переходим к следующему шагу
...
; достигает верхней границы дисплея или встречает пустой ряд, переходим к следующей фазе.
...
block_not_full  db  0                       ; Флаг неполноты правой части игрового поля
block_need_drop db  0                       ; Флаг необходимости обрушения башни

; Подготавливаем указатели и флаги
block_start:        clr a
...
; Проверяем, заполнен ли ряд
                    not a                   ; Признак неполноты левой части игрового поля
                    ld b, block_not_full    ; Признак неполноты правой части игрового поля
...
; Данные о каждой фигуре: адрес с данными о повороте фигуры и её стартовое состояние
minos           db  0x00,                   ; Фигура O
                    0b00001100,
                    0b00001100,
                    turn_i0,                ; Фигура I
                    0b00000000,
