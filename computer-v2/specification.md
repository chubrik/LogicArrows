...
перемещение курсора стрелками. Поддерживает вывод пиксельной графики *(в разработке).*
...
[`cp1251`](https://ru.wikipedia.org/wiki/Windows-1251).

Байты, отправленные по адресу `3D`, накапливаются в буфере из 6 байт. При заполнении буфера все байты
выводятся в терминал как графический символ, после чего буфер очищается *(в разработке).*
