...
переключения банков памяти через порт `3F`. Если в этот порт записать число, диапазон адресов
`80...FF` переключится на банк соответствующего номера. Число `0` как исключение ссылается на банк
№ 1. Номер банка памяти не может быть предзаписан на дискете, а управляется только во время
