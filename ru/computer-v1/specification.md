...
`C` `D`. Вычислительная часть состоит из дополнительного `IR`, флагов `Z` `S` `C` `O`, блока
...
адрес доступа к памяти также представляет собой 1 байт. Порт ввода `3E` подключён к клавиатуре. Порт
вывода `3F` позволяет переключаться между различными средствами вывода. Диапазон адресов `40...7F`
является совмещённой видеопамятью.
...
При переключении средств вывода через порт `3F` текущее изображение на дисплее сохраняется, но
меняется эффект от последующих записей в экранную область памяти. Переключить вывод на дисплей
можно не только программно, но и прямо во время загрузки с дискеты, что позволяет выводить
стартовое изображение-заставку.
