...
- Откройте [онлайн-компилятор](https://chubrik.github.io/arrows-compiler/#cpu=v1) и в его левой
  части напишите программу.
...
`O`. Каждый регистр хранит 1 байт и имеет универсальное назначение. Флаги отражают результат
...
- **Флаг O (переполнение)** показывает, что в знаковой арифметике результат вышел из диапазона
  `−128...+127`.
<br><br><br>
...
MY_CONST equ 196                ; Объявляем константу MY_CONST со значением 196 при помощи ключевого
                                ...
                                ; можно использовать символ в кавычках ("Д"), что эквивалентно числу
                                ; в кодировке cp1251.
...
Инструкция | Описание | Пример кода
---|---|---
ld ***X*** | Загружает в ***X*** значение из памяти, используя операнд как адрес | `ld a, 0xC4`
ld ***X***, ***Y*** | Загружает в ***X*** значение из памяти, используя ***Y*** как адрес | `ld a, b`
ldi ***X*** | Загружает операнд непосредственно в ***X*** | `ldi a, 196`
st ***X*** | Сохраняет значение ***X*** в память по адресу из операнда | `st a, 0xC4`
st ***X***, ***Y*** | Сохраняет значение ***X*** в память по адресу из ***Y*** | `st a, b`
jmp | Безусловный переход по адресу из операнда | `jmp 0xC4`
jmp ***X*** | Безусловный переход по адресу из ***X*** | `jmp a`
j***F*** | Переход по адресу из операнда, если ***F*** = 1 | `jz 0xC4`
jn***F*** | Переход по адресу из операнда, если ***F*** = 0 | `jnz 0xC4`
j***F*** ***X*** | Переход по адресу из ***X***, если ***F*** = 1 | `jz a`
jn***F*** ***X*** | Переход по адресу из ***X***, если ***F*** = 0 | `jnz a`
rnd ***X*** | Генерирует случайное значение в ***X*** | `rnd a`
hlt | Завершает выполнение программы | `hlt`
...
and ***X***, 0 | Обнуляет ***X*** и обновляет флаги | Z, S
...
or ***X***, 0<br> xor ***X***, 0 | Не производят вычислений, а лишь обновляют флаги по значению ***X*** | Z, S
...
add ***X***, 0<br> adc ***X***, 0<br> sub ***X***, 0<br> sbb ***X***, 0 | Соответствуют аналогам выше, но с 0 в качестве второго операнда | Z, S, C, O
...
exp ***X*** | Делает все биты равными флагу `C` | Z, S
...
rcl ***X*** | Сдвигает все биты на один влево, правый бит берёт из флага `C` | Z, S, C
rcr ***X*** | Сдвигает все биты на один вправо, левый бит берёт из флага `C` | Z, S, C

<br>

### Таблица инструкций
Каждая инструкция имеет свой код (opcode). К примеру, `adc a, d` имеет код `2B` (ряд, затем
колонка).
<table>
  <thead>
    <tr>
...
Некоторые инструкции имеют несколько кодов, дающих одинаковый результат (например, `mov a, 0`
встречается и под `00`, и под `04`). Это связано с особенностями технической реализации,
унаследованными от [процессора-прототипа](https://habr.com/p/590821/).
