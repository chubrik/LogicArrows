...
; ##           Исходный код игры "Бой в космосе" для компьютера из логических стрелочек           ##
...
random:         ldi c, display          ; Ссылка на начало области дисплея как верхняя граница
                                        ;   заполнения врагами
...
; Выводим счёт врагов на цифровой индикатор
score:          ld a, win_left          ; Читаем счётчик врагов. Если игра проиграна, эта инструкция
                                        ;   будет перезаписана на "hlt".
                ldi d, display          ; Ссылка на начало области дисплея, в т.ч. для блока "win"
                ...
                add a, 0                ; Если счётчик врагов достиг нуля, игра выиграна
                jz win

; Уменьшаем счёт ходов
step:           ldi b, step2            ; Запоминаем адрес для перехода к следующему ходу для блоков
                                        ;   "keys", "left", "right"
step2:          ld a, step_left
                dec a
                js level                ; Если ходов не осталось, переходим на следующий уровень
                st a, step_left
                jmp keys                ; Переходим через зарезервированную область памяти
...
; Регистр B уже содержит адрес для возврата к step2.
keys:           mov a, 0
...
left:           ld a, 0x5E
                shl a
                jc b                    ; Если мы у левого края, переходим к следующему ходу
                ldi d, 0x5F
                ldi c, 4
...
right:          ld a, 0x5F
                shr a
                jc b                    ; Если мы у правого края, переходим к следующему ходу
                ldi d, 0x5C
                ldi c, 4
...
; Попадание. Убираем врага с дисплея, уменьшаем счёт врагов и переходим к следующему ходу.
fire_hit:       ld a, d
                xor a, b
                st a, d
                ld a, win_left
...
; Переходим на следующий уровень. Здесь враги станут ближе к нам, а число доступных ходов
; уменьшится.
level:          ld a, step_cnt          ; Уменьшаем число доступных ходов на два
                ...
                ld b, 0x5B
                or a, b
                jz scroll               ; Если в третьем снизу ряду нет врагов, переходим к сдвигу
                                        ;   карты
                inc c                   ; Игра проиграна, смещаем нижнюю границу сдвига карты на
                                        ;   один ряд ниже
                inc c
                ldi a, 0xEC             ; В блоке "score" первую инструкцию перезаписываем на "hlt"
                st a, score

; Сдвигаем всех врагов ближе к нам.
; Регистр C уже содержит указатель на дисплее на нижнюю границу для сдвига карты.
...
win_loop:       ld a, c
                st a, d
                inc d
                inc c                   ; После 0xFF значение перейдёт в 0x00, что означает конец
                                        ;   вывода
                jnz win_loop
