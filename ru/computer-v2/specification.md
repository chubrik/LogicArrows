...
регистров `A` `B` `C` `D`, флагов `Z` `S` `C` `O`, многофункционального сумматора, битового сдвига и
других мелких механизмов.
...
<img src="../../computer-v2/img/cpu.jpg" width="60%" alt="Процессор">
<br><br>


## Оперативная память
Памятью компьютера является RAM объёмом 1 КБ с возможностью расширения до 32 КБ. Единицей хранимой
информации является 1 байт, адрес доступа к памяти также представляет собой 1 байт.

Т. к. 8-битный адрес может обеспечить доступ лишь по 256 адресам RAM, существует система
переключения банков памяти. По умолчанию подключён банк № 1, но если в порт `3F` записать число,
диапазон адресов `80...FF` переключится на банк соответствующего номера. Число `0` как исключение
подключает банк № 1.

<img src="../../computer-v2/img/ram.jpg" width="80%" alt="Оперативная память">
<br><br>


## Клавиатура
Полноформатная клавиатура, приближенная к реальному ПК. Имеет латинскую и кириллическую раскладки,
однократный и постоянный верхний регистр. Все режимы отображаются на соответствующих клавишах. Также
имеет индикатор, подсказывающий, когда можно вводить следующий символ.
...
[`cp1251`](https://ru.wikipedia.org/wiki/Windows-1251). Шесть байт, последовательно отправленных по
адресу `3D`, выводятся в терминал как один графический символ.
