...
; Константы
BANK_INNER      equ 1                       ; Банк памяти № 1
BANK_MAIN       equ 2                       ; Банк памяти № 2
BANK_OUTER      equ 3                       ; Банк памяти № 3



;===================================================================================================
; Общая область памяти - адресное пространство 0x00...0x7F.
; Содержит: код вывода названия игры в терминал, переключатель банков памяти, порты, переменные,
; память дисплея, кэш для следующего кадра. После вывода названия игры в терминал, вся область
; памяти 0x00...0x2F перетирается и далее используется для хранения счётчиков соседей у клеток.
; Счётчики разбиты на три группы по 16, где каждая группа отвечает за свой ряд на дисплее.
;===================================================================================================

; Старт игры
                    jmp title_show          ; Переходим через строку
title           db  "ьнзиЖ аргИ ",          ; Название игры по-русски (перевёрнуто)
                    "efiL fo emaG"          ; Название игры по-английски (перевёрнуто)

; Готовим вывод названия игры в терминал
title_show:         ldi b, " "
                    st b, title - 1         ; Дополняем пробелом строку с названием игры
                    ldi a, terminal
                    ldi c, title_show - 1   ; Указатель на последний символ строки. В течение цикла
...
; Готовим регистры C и D для перехода между банками памяти, а регистры A и B для блока "main_rnd"
                    ldi a, display
...
bcd             db  255, 0                  ; Счётчик кадров. При первой инкрементации значение
                                            ;   изменится на 0.
terminal        db  0
incrementor     db  0                       ; Счётчик для некоторых циклов
out             db  0b00110001              ; Порт вывода, подключены цветной дисплей и терминал
...
; ниже и требует сброса своих счётчиков. Когда мы достигнем нижнего ряда, все ссылки автоматически
; получат свои изначальные значения.
...
; Регистр C уже содержит указатель на дисплее.
; Регистр D уже содержит ссылку на начало области счётчиков для нижнего ряда на дисплее.
...
; Обновляем счётчик кадров на цифровом индикаторе. На этом вся работа над кадром завершена, и мы
; переходим к работе над следующим кадром.
main_count:         ld a, bcd
                    inc a
                    st a, bcd
...
; Готовим к обработке нижний ряд на дисплее. В переменную lower_area временно записываем ссылку
; upper_area. Это позволяет использовать единый код программы как для верхнего ряда на дисплее, так
; и для нижнего.
