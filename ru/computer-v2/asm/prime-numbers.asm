...
factor: inc d               ; Смещаем указатель на следующее простое число на дисплее
        ...
        jc prime            ; Если множитель больше половины числа-кандидата, то дальше подбирать
                            ;   множители нет смысла - мы уже нашли новое простое число
        mov a, b            ; Копируем число-кандидат
...
loop:   sub a, c            ; Вычитание
        ...
        jnc loop            ; Если нет переноса, продолжаем вычитать

        jmp factor          ; Если есть перенос, переходим к следующему множителю
