...
buffer_ptr      db  INI_BUFFER_PTR          ; Указатель в буфере (0x00...0x07)
display_ptr     db  INI_DISPLAY_PTR         ; Указатель на дисплее (0x67...0x7F, всегда нечётный)
buffer_ptr_2    db  0x00                    ; Дополнительный указатель в буфере
...
bcd             db  0, 0
terminal        db  0, 0
in_out          db  0b00110001              ; Порт ввода/вывода, подключены цветной дисплей и
                                            ;   терминал
...
; в буфере, как бы уменьшая и размер области фигуры на дисплее, и размер буфера.
...
turn_data_ptr   db  0                       ; Указатель в данных о следующем повороте фигуры
...
turn_check:         ldi c, 8                ; В крайней правой колонке поворот невозможен
                    ldi d, 0x05             ; В двух нижних рядах поворот невозможен
                    jmp turn_check_end

turn_check_i:       ldi c, 7                ; В двух правых колонках поворот невозможен
                    ldi d, 0x07             ; В трёх нижних рядах поворот невозможен
...
; Готовим отдельный указатель в буфере, не зависящий от положения фигуры на дисплее
                    ldi a, buffer_end
                    st a, buffer_ptr_2
...
block_start:        clr a
                    ...
                    ldi c, 256 - 32
                    add c, d
                    st c, display_ptr_2     ; Указатель в красной части области дисплея
