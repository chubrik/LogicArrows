...
; Выводим стартовый счёт на цифровой индикатор
score:          ld a, bcd
                st a, bcd
                ldi a, step
...
step_loop:      ld a, step_left         ; Уменьшаем счёт ходов
                dec a
                ...
                jz fire
                xor c, a
                jmp step_end            ; Переходим через зарезервированную область памяти

; Переменные и порты
bcd         db  WIN_LEFT, 0             ; Счётчик врагов, оставшихся до победы
...
in_out      db  0b00010100              ; Порт ввода/вывода, подключены монохромный дисплей и
                                        ;   цифровой индикатор
...
fire_hit:       ld a, d
                xor a, b
                st a, d
                ld a, bcd               ; Уменьшаем счёт врагов на цифровом индикаторе
                dec a
                st a, bcd
                jnz step                ; Если счётчик врагов не достиг нуля, переходим к следующему
                                        ;   ходу
...
level_lose:     inc c
                ...
                inc d
                inc d
                ldi a, 0x01             ; Код инструкции "hlt". Записываем его в то место, где
                                        ;   закончится заполнение верхнего ряда.
                st a, random_jmp

; Сдвигаем всех врагов ближе к нам.
; Регистр C уже содержит счётчик для выхода из цикла.
; Регистр D уже содержит указатель на дисплее на нижнюю границу для сдвига карты.
scroll:         mov b, d
                ld a, b
                inc b
                inc b
