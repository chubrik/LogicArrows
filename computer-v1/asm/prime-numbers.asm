...
next:   inc b           ; Дважды увеличиваем число-кандидат - идём только по нечётным числам
        inc b
...
test:   inc d           ; Смещаем указатель на следующее простое число на дисплее
        inc d
        ...
        js prime        ; Если множитель больше половины числа-кандидата, то результат будет меньше
                        ;   нуля, и значит дальше подбирать множители нет смысла - мы уже нашли
                        ;   новое простое число
        mov a, b        ; Копируем число-кандидат

; Цикл вычитания множителя из числа-кандидата
loop:   sub a, c        ; Вычитание
        jz next         ; Если результат равен нулю, значит число не простое, переходим к следующему
                        ;   кандидату
...
prime:  ld d, last      ; Читаем указатель на последнее найденное простое число на дисплее
        ...
        st b, d         ; Сохраняем новое простое число на дисплей
