...
test:   inc d               ; Смещаем указатель на следующее простое число на дисплее
        ...
        js prime            ; Если множитель больше половины числа-кандидата, то результат будет
                            ;   меньше нуля, и значит дальше подбирать множители нет смысла — мы уже
                            ;   нашли новое простое число
        mov a, b            ; Копируем число-кандидат
...
prime:  ld d, last          ; Читает указатель на последнее найденное простое число на дисплее
