...
указателя инструкции `IP`, регистра инструкции `IR`, блока управления и 4 свободных регистров `A`
`B` `C` `D`. Вычислительная часть состоит из регистра инструкции `IR`, 4 флагов `Z` `C` `S` `O`,
блока управления, многофункционального сумматора и ряда других мелких механизмов.
...
Адреса `40...7F` являются совмещённой видеопамятью. Отправка данных по этим адресам может
...
## Клавиатура
После нажатия на любую клавишу, её код может быть прочитан программой из порта `3E`. Для
