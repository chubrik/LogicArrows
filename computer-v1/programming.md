...
## Общие принципы
Программа состоит из последовательности команд и набора данных. Каждая команда занимает в памяти
1 байт. Некоторые команды требуют дополнительный аргумент, который также занимает 1 байт и
располагается в памяти сразу вслед за командой. Процессор выполняет команды друг за другом, начиная
с адреса `00`. Согласно программе, процессор также может совершать переходы к различным участкам
кода, тем самым создавая циклы, подпрограммы и пр.
...
выполнения операций вычисления и могут быть использованы как условие для совершения переходов:
...
### Команды управления
Команды из этого набора отвечают за операции ввода/вывода и переходы. В таблице ниже используются
...
jmp | Совершает безусловный переход, используя аргумент как адрес
...
### Таблица команд
Каждая команда имеет свой уникальный HEX-код. К примеру, `adc a, d` имеет код `2B` (ряд, затем
колонка).
<table>
  <thead>
    <tr>
