...
KEY_FIRE    equ 0x20                    ; Код клавиши "Пробел"
...
; Регистр D уже содержит адрес области на дисплее.
random:         rnd a                   ; Работаем с правой частью дисплея
                ...
                shl a                   ; Отодвигаем биты от края дисплея, где невозможно
                                        ;   произвести выстрел
                st a, d
                dec d
...
score:          ld a, win_left
                st a, win_left
                ldi a, step
                st a, random_jmp +1     ; Т.к. этот блок кода нужен лишь однократно, делаем так,
                                        ;   чтобы в дальнейшем его перепрыгивать

; Опрашиваем клавиатуру на нажатие одной из управляющий клавиш
step:           ldi b, KEY_LEFT
                ldi c, KEY_RIGHT
                ldi d, KEY_FIRE
...
step_loop:      ld a, step_left         ; Уменьшаем счётчик ходов
                dec a
                js level                ; Если ходов не осталось, переходим к следующему уровню
                st a, step_left
                ld a, in_out
                xor d, a
                jz fire
...
win_left    db  30                      ; Порт вывода на цифровой индикатор, используется как
                                        ;   счётчик врагов, оставшихся до победы
void1       db  0

step_cnt    db  14                      ; Количество доступных ходов на текущем уровне
step_left   db  14                      ; Счётчик оставшихся ходов на текущем уровне
in_out:     db  0b_00000110             ; Порт вывода, подключён монохромный дисплей и цифровой
                                        ;   индикатор
...
; Мы выиграли! Выводим на дисплей финальную заставку-приз и завершаем выполнение программы.
win:            ldi b, display
                ldi c, win_image
                ldi d, win_loop
...
level:          ld a, step_cnt          ; Уменьшаем число доступных ходов на два
                ...
                st a, step_cnt
                st a, step_left
                ld a, 0x5A              ; Определяем, если ли хоть один враг в нижнем ряду
                ld b, 0x5B
                ldi c, 26
                ldi d, 0x59
                or a, b
...
; Игра проиграна. Осталость сдвинуть всех врагов ещё ближе и остановить игру.
level_lose:     inc c
                inc c
                inc d
                inc d
...
; Заставка-приз для победителя
win_image   db  0b_00000000, 0b_00000000,
